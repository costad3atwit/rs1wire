...
Full planning notes, claims, sources, and to-dos live in `docs/planning.md`. Read it before making design decisions. If a decision here changes something in that file, update it and add a dated line to its Changelog section.

## Target environment

- Raspberry Pi 5 running Raspberry Pi OS with the `w1-gpio` and `w1-therm` kernel modules (exact OS/kernel versions: TBD, record them here once fixed)
- Sensors: DS18B20 (thermal, multiple on one bus); DS2413 (non-thermal) on a separate GPIO pin and w1-gpio overlay instance
- Known risk: a Pi 5/RP1 w1-gpio instability bug may exist on some kernels
...
- The contribution is NOT "consuming sysfs" or "using bulk read"; prior art exists for both (w1thermsensor, w1_therm_reader, w1-therm-api, wb-mqtt-w1).
- The contribution IS: discovery + bulk conversion + configuration in a maintained Rust crate, a bulk-read API that cannot return stale data by construction, and Python bindings that release the GIL.
...
- **Stale-read safety.** After a `therm_bulk_read` trigger, the kernel returns the value from trigger time if a sensor isn't read promptly. The bulk-read API must make stale access impossible or an explicit error (e.g. a result/handle type consumed by reading, tied to one trigger).
...
- Regime 2: many sensors at fixed counts (e.g. 1, 5, 10, 20). Baselines: w1thermsensor sequential, w1thermsensor async/concurrent, w1-therm-api bulk, this crate native, this crate via Python
- Regime 3 (optional): DS2413, no conversion delay, isolates per-call overhead
- Metrics: wall-clock time, CPU usage, error rate
- Record kernel version, OS version, sensor count, and resolution with every result
